                ...
                <div class="content-tabs">
                    ...
                    <nav class="page-tabs J_menuTabs">
                        ...
                        <!--    <a href='/szny/index.php/Admin/Twicedatah/tableindex?rgn_atpid=<?php echo ($rgn_atpid); ?>' target="_self" class='J_menuTab'>时数据</a>-->
                            <a href='/szny/index.php/Admin/Twicedatad/tableindex?rgn_atpid=<?php echo ($rgn_atpid); ?>' target="_self" class='J_menuTab'>日数据</a>
                            <a href='/szny/index.php/Admin/Twicedatam/tableindex?rgn_atpid=<?php echo ($rgn_atpid); ?>' target="_self"  class='J_menuTab'>月数据</a>
                            <a href='/szny/index.php/Admin/Twicedata/tableindex?rgn_atpid=<?php echo ($rgn_atpid); ?>' target="_self" class='J_menuTab active'>年数据</a>
                        </div>
                    </nav>
            ...
            <div class="row row-lg">
                <div class="col-sm-12">
                    <div id="atpbiztoolbar">
                        ...
                        <input type="text" id="starttime" class="form-control" value="<?php echo ($starttime); ?>" onClick="WdatePicker({dateFmt:'yyyy'})" style="width: 120px;" placeholder="输入结束日期">
                        <label class="control-label">结束日期：</label>
                        <input type="text" id="endtime" class="form-control" value="<?php echo ($endtime); ?>" onClick="WdatePicker({dateFmt:'yyyy'})" style="width: 120px;" placeholder="输入结束日期">
...
<script>
    $(function () {
        $('#atpbiztable').bootstrapTable({
            url: '/szny/index.php/Admin/Twicedata/getData?rgn_atpid=<?php echo ($rgn_atpid); ?>',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
...
//            height: 600,                        //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
            uniqueId: "d2y_atpid",                     //每一行的唯一标识，一般为主键列
          ...
          //  detailView: true,                   //是否显示父子表
            ...
            columns: [
                [
                    ...
                    {field: 'd2y_dt', title: '时间', sortable: true},
                    ...
                    {field: 'd2y_atpid', title: '主键', sortable: true, visible:false},
                    {field: 'd2y_atpcreatedatetime', title: '创建时间', sortable: true, visible:false},
                    {field: 'd2y_atpcreateuser', title: '创建人', sortable: true, visible:false},
                    {field: 'd2y_atplastmodifydatetime', title: '最后修改时间', sortable: true, visible:false},
                    {field: 'd2y_atplastmodifyuser', title: '最后修改人', sortable: true, visible:false},
                    {field: 'd2y_atpstatus', title: '状态', sortable: true, visible:false},
                ]
            ],
            onDblClickRow: function (row) {
/*
                $("#sys_dlg").load('/szny/index.php/Admin/Twicedata/edit?id=' + row['atpid'], function() {
...
//                console.log(name+order);
    ...
    $('#sys_search').on('click',function() {
       ...
       window.location.href='/szny/index.php/Admin/Twicedata/tableindex?endtime='+endtime+'&starttime='+starttime+'&rgn_atpid='+regionid;
    ...
    function submitdata()
    {
        $("#sys_dlg_form").submit(function(e)
        {
            ...
            $.ajax({
                url: '/szny/index.php/Admin/Twicedata/submit',
                type: 'POST',
                data:  formData,
    ...
    $('#sys_update').on('click',function() {
        ...
        if(tablerow.length!=1)
        {
            ...
            $("#sys_dlg").load('/szny/index.php/Admin/Twicedata/edit?id=' + tablerow[0]['atpid'], function() {
                $('#sys_dlg_submit').on('click',submitdata);
                $("#sys_dlg").modal({backdrop: false});
            });
        }
    ...
    function updateInRow(id)
    {
        ...
        $("#sys_dlg").load('/szny/index.php/Admin/Twicedata/edit?id=' + id, function() {
            $('#sys_dlg_submit').on('click',submitdata);
            $("#sys_dlg").modal({backdrop: false});
        });
    }
    ...
    function delInRow(id)
    {
        if (confirm('确认删除数据?')) {
            var ids = [];
            ids.push(id);
            $.post('/szny/index.php/Admin/Twicedata/del', {ids: ids.join(',')}, function (rep, status) {
                if ('' == rep) {
                    $('#atpbiztable').bootstrapTable('refresh')
                }
                else {
