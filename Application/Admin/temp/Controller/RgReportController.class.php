...
class RgReportController extends BaseAuthController{
    public function index(){
        $this->logSys(session('emp_atpid'), "访问日志", "访问页面：$this->ATPLocationName / 【报表】");
        ...
        if (null != $start && null != $end) {
            ...
            if ( 4 == $sum){
                ...
                $sql_select_columns = "select t.d2d_dt from szny_data2day t where t.d2d_dt between '".$start ."' and '".$end ."' group by t.d2d_dt order by t.d2d_dt desc";
                $Result_column = $Model_columns->query($sql_select_columns);
                $columns = [];
                foreach ($Result_column as $k => $v){
    ...
    public function getData(){
        ...
        $select_param_id = I('get.selectParam','');
        ...
        foreach ($Result as $k => &$v)
        {
            ...
            if (4 == $sum) {
                ...
                $sql_select_sum = $sql_select_sum . " group by t.d2m_dt order by t.d2m_dt desc";
                $Result_sum = $Model_sum->query($sql_select_sum);
                $time = [];
                $series_array = [];
                $series_array['name'] = $v['rgn_name'];
                ...
                $sql_select_sum = $sql_select_sum . " group by t.d2d_dt order by t.d2d_dt desc";
