...
class RgAlarmHistoryController extends BaseAuthController
{
    public function index()
    {
        $this->logSys(session('emp_atpid'),"访问日志","访问页面：$this->ATPLocationName / 【历史报警记录】");
        $this->display();
    }
...
//        }
        ...
        if (null != $queryparam['alm_confirmstatus']) {
            $searchcontent = trim($queryparam['alm_confirmstatus']);
            $sql_select = $this->buildSql($sql_select, "t.alm_confirmstatus like '%" . $searchcontent . "%'");
            $sql_count = $this->buildSql($sql_count, "t.alm_confirmstatus like '%" . $searchcontent . "%'");
        }
