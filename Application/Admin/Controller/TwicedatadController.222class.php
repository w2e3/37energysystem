...
left join szny_device t1 on t1.dev_atpid = t.d2y_deviceid left join  szny_region t2 on t2.rgn_atpid=t.d2y_regionid where d2m_atpid= '".$id."'";
    ...
    public function getData(){
      ...
      $rgn_atpidlist = implode(',', $trgn_atpid);
        ...
        if($rgn_atpidlist){
            $sql_select = $this->buildSql($sql_select, "t.d2d_regionid in(".$rgn_atpidlist.")");
            $sql_count = $this->buildSql($sql_count, "t.d2d_regionid  in(".$rgn_atpidlist.")");
        }elseif($regionid) {
...
//        dump($Result_rel);
    ...
    function exp(){
         $this->logSys(session('emp_atpid'),"访问日志","访问页面：【数据管理】 / 【二次数据管理】 / 【天数据】 / 【导出】");
        ...
        $naem='数据导出'.date('Y-m-d',time());
        $letter = array('A', 'B', 'C', 'D', 'E', 'F', 'G','H','I');
        $tableheader = array('设备名', '设备编号','时间','用水量-吨(T)','用暖量-千瓦时(Kwh)','用冷量-千瓦时(Kwh)','电压-伏特(V)','电流-安培(A)','电量值-千瓦时(Kwh)');
...
//        dump($Result_rel);
    ...
    foreach($Result as $orderkey=>$orderval){
        ...
        header('Content-Disposition:attachment;filename="'.$naem.'.xlsx"');
