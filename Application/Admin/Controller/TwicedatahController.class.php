...
class TwicedatahController extends BaseController
{
    public function index()
    {
        $this->logSys(session('emp_atpid'),"访问日志","访问页面：【数据管理】 / 【二次数据管理】 / 【时数据】");
        $Model = M();
        $sql_select ="
        select 
        * 
...
//        dump($treedatas);
    ...
    function update(){
      ...
      /*  $sql_select = "
				...
				";
        $sql_select = $this->buildSql($sql_select, "t.d2mdf_atpid='".$d2mdfid."'");
        $Result = $Model->query($sql_select);

        $data2modifydetail=$Model->query("select
                      t.*,t2.rgn_name,t3.dev_name
                      from szny_data2modifydetail t
                      left join szny_data2hour t1 on t1.d2h_atpid=t.d2mdfd_data2hourid
                      left join szny_region t2 on t2.rgn_atpid=t1.d2h_deviceid
                      left join szny_device t3 on t3.dev_atpid=t1.d2h_atpid
                      where d2mdfd_data2modifyid='".$d2mdfid."'
                      ");

        $this->assign('Resultd', $data2modifydetail);
...
left join szny_device t1 on t1.dev_atpid = t.d2h_deviceid left join  szny_region t2 on t2.rgn_atpid=t.d2h_regionid whered2h_atpid= '".$id."'";
        $data2year=$Model->query($sql_select);
     if(empty($data2year['0']['d2h_atpid'])){
         $sql_select = "
select
  t.*,t2.rgn_name,t1.dev_code
from szny_data2month t
left join szny_device t1 on t1.dev_atpid = t.d2h_deviceid left join  szny_region t2 on t2.rgn_atpid=t.d2h_regionid where d2h_atpid= '".$id."'";
    ...
    */
    function edit(){
        $this->logSys(session('emp_atpid'),"访问日志","访问页面：【数据管理】 / 【二次数据管理】 / 【时数据】 / 【发起审批】");
        $id = I('get.id');
        $Model = M();
        ...
        if($id){
            $sql_select = $this->buildSql($sql_select, "t.d2h_atpid='$id'");
        }else{
    ...
    function add(){
        $this->logSys(session('emp_atpid'),"访问日志","访问页面：【数据管理】 / 【二次数据管理】 / 【时数据】 / 【添加】");
...
//        //$data_data2modify=$Model_data2modify->create();
        $month=$Model->where("d2h_atpid='%s'",$_POST['d2h_atpid'])->find();
...
//        //插入二次数据纠正表
        ...
        foreach($keys as $key=>$val){
            if(!empty($val)){
                ...
                $data2modifydetaildata['d2mdfd_data2hourid'] =$data['d2h_atpid'];
    ...
    public function  submit(){
        ...
        $sql_select = "select t2.dev_atpid from szny_region t left join szny_device t2 on t.rgn_deviceid=t2.dev_atpid where t2.dev_status='启用' and t2.dev_atpstatus is null and t.rgn_atpid='".$data['d2h_regionid']."' ";
        $Resultdata = $Model->query($sql_select);
        $data['d2h_deviceid']=$Resultdata['0']['dev_atpid'];
        if (null == $data['d2h_atpid']) {
            $data['d2h_atpid']=$this->makeGuid();
            $data['d2h_atpcreatedatetime'] = date('Y-m-d H:i:s', time());
            $data['d2h_atpcreateuser'] = I('session.u_account', '');
            $data['d2h_atplastmodifydatetime'] = date('Y-m-d H:i:s', time());
            $data['d2h_atplastmodifyuser'] = I('session.u_account', '');
            $data['d2h_dt'] =  I('post.time','');
...
//        }
        ...
        if($regionid){
            $sql_select = $this->buildSql($sql_select, "t.d2h_regionid='$regionid'");
            $sql_count = $this->buildSql($sql_count, "t.d2h_regionid ='$regionid'");
        }else{
         //   $sql_select = $this->buildSql($sql_select, "t.data_regionid in('".$arr."')");
          //  $sql_count = $this->buildSql($sql_count, "t.data_regionid  in('".$arr."')");
        }
