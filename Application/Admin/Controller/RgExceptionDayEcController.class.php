...
class RgExceptionDayEcController extends BaseAuthController
{

    public function index(){
        $this->logSys(session('emp_atpid'),"访问日志","访问页面：$this->ATPLocationName / 【日报异常】");
...
group by LEFT (t.alm_datetime, 10)
        ...
        if (null != $queryparam['sort']) {
            $sql_select = $sql_select . " order by LEFT (t.alm_datetime, 10) "  . $queryparam['sortOrder'] . ' ';
        } else {
            $sql_select = $sql_select . " order by LEFT (t.alm_datetime, 10) desc";
        }
