...
class TwicedatamController extends BaseController
{
    public function index()
    {
        $this->logSys(session('emp_atpid'),"访问日志","访问页面：【数据管理】 / 【二次数据管理】 / 【月数据】");
        $Model = M();
        $sql_select ="
        select 
        * 
...
//        dump($treedatas);
    ...
    function add(){
        $this->logSys(session('emp_atpid'),"访问日志","访问页面：【数据管理】 / 【二次数据管理】 / 【月数据】 / 【编辑】");
        $id = I('get.id');
        $Model = M();
        ...
        if($id){
            $sql_select = $this->buildSql($sql_select, "t.d2m_atpid='$id'");
        }else{
    ...
    public function  submit(){
        $Model = M('data2month');
        $data = $Model->create();
            if (null != $data['d2m_atpid']) {
                $data['d2m_atplastmodifydatetime'] = date('Y-m-d H:i:s', time());
                $data['d2m_atplastmodifyuser'] = I('session.u_account', '');
                $asd=$Model->where("d2m_atpid='%s'", array($data['d2m_atpid']))->save($data);
            }
   }
    public function  addsubmit(){
      ...
      //  $data2modifydetail=$Model_data2modifydetail->create();
        ...
        $month=$Model->where("d2m_atpid='%s'",$_POST['d2m_atpid'])->find();
...
//        //插入二次数据纠正表
        ...
        $datadata2modify['d2mdf_name']=$data['d2mdf_name'];
        $datadata2modify['d2mdf_desc']=$data['d2mdf_desc'];
        ...
        foreach($keys as $key=>$val){
           if(!empty($val)){
                ...
                $data2modifydetaildata['d2mdfd_data2hourid'] =$data['d2m_atpid'];
...
//        }
        ...
        if($regionid){
            $sql_select = $this->buildSql($sql_select, "t.d2m_regionid='$regionid'");
            $sql_count = $this->buildSql($sql_count, "t.d2m_regionid ='$regionid'");
        }else{
         //   $sql_select = $this->buildSql($sql_select, "t.data_regionid in('".$arr."')");
          //  $sql_count = $this->buildSql($sql_count, "t.data_regionid  in('".$arr."')");
        }
